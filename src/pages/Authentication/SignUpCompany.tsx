...
const SignUpCompany = () => {
  const [user, setUser] = useState({ email: '', password: '', re_password: '', id_company: '', company_name: '', name: 'admin', permissions: true, status: true })
  ...
  const handlercompany = async () => {
    const { error } = await supabase
      .from('company')
      .insert({ id_company: user.id_company, name: user.company_name, status: user.status })
    if (error) {
      console.log(error)
    }
  }
